// watch.js (v2 — secure direct MP4 playback, adaptive loading, no fixed timeout)
(function() {
    'use strict';

    const BUILD_VERSION = 'v20260923-2';
    ...
    // ================== VIDEO ATTACH — ADAPTIVE, NO FIXED TIMEOUT ==================
    //
    // Njira yamakono:
    //   • Siyigwiritsa ntchito fixed timeout ya 30s
    //   • Imayang'ana `progress` events (bytes akubwera)
    //   • Imayang'ana `readyState` kuchokera pa video element
    //   • Ikangolephera kwa nthawi yaitali POPANDA kusuntha, ndipamene imalephera
    //   • Imayesanso kangapo (auto-retry) ngati network yalephera
    //
    ...
    function attachVideo(url, opts = {}) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const STALL_MS = 20000;     // 20s popanda byte imodzi = stalled
            const ABSOLUTE_MAX_MS = 5 * 60 * 1000; // 5 min hard cap kuti tipewe kutsekelera
            ...
            const onErr = () => {
                ...
                settle(reject, new Error(`${label} failed: ${code} — ${msg} | URL: ${url.slice(0, 100)}...`));
            };
            const onProgress = () => {
                ...
                if (buf && buf.length > 0) {
                    ...
                    // bytesLoaded approximate kudzera mu duration ratio
                    if (video.duration > 0) {
                        totalBytes = Math.max(totalBytes, Math.round((end / video.duration) * 100));
                    }
                }
            };
            const onLoadedStart = () => {
                console.log(`[Akimark] ${label} started loading`);
                lastProgressAt = Date.now();
            };
            const onWaiting = () => {
                // video element ikudikira data
                lastProgressAt = Date.now();
            };
            ...
            const stallTimer = setInterval(() => {
                ...
                // Ngati video yayamba kale kusewera, si vuto
                if (video.readyState >= 1) return;

                if (stalledFor > STALL_MS) {
                    if (retries < maxRetries) {
                        retries++;
                        console.warn(`[Akimark] ${label} stalled for ${Math.round(stalledFor/1000)}s — retry ${retries}/${maxRetries}`);
                        lastProgressAt = Date.now();
                        video.load();
                        return;
                    }
                    settle(reject, new Error(
                        `${label} stalled — no data received for ${Math.round(stalledFor/1000)}s.\n` +
                        `• URL: ${url}\n` +
                        `• readyState: ${video.readyState}\n` +
                        `• networkState: ${video.networkState}\n` +
                        `• Hint: Worker kapena R2 ikulephera kuyankha. Yang'anani Cloudflare Worker /health.`
                    ));
                }
                if (totalElapsed > ABSOLUTE_MAX_MS) {
                    settle(reject, new Error(`${label} exceeded hard cap (${ABSOLUTE_MAX_MS/60000} min)`));
                }
            }, 1000);
    ...
    async function init() {
        ...
        try {
            ...
            console.log('[Akimark] Session OK:', {
                ...
                streamUrl: session.streamUrl?.slice(0, 120) + '...',
            });

            currentClientKey = session.clientKey || null;

            // ⚠️ YANG'ANANI: streamUrl iyenera kukhala mp4, osati m3u8
            if (session.streamUrl && session.streamUrl.includes('.m3u8')) {
                throw new Error(
                    `🔴 VUTO: Session API ikubweletsa HLS URL (m3u8) osati MP4!\n` +
                    `URL: ${session.streamUrl}\n` +
                    `Chitani: Konzani player-session-api kuti ibweletse direct MP4.`
                );
            }

            console.log('[Akimark] Attaching video stream (direct MP4)…');
    ...
    video.addEventListener('error', () => {
        showLoader(false);
        const e = video.error;
        console.error('[Akimark] Video error event:', e);
    });
    ...
    // Error watsopano — umadziwa kuti vuto ndi worker, watch.js, kapena session
    function showDetailedError(err) {
        const msg = err?.message || String(err);
        let diagnostic = '';
        if (msg.includes('HLS')) {
            diagnostic = '🔴 VUTO: watch.js wakale ukugwirabe ntchito (cache). Hard refresh kapena clear cache.';
        } else if (msg.includes('ERR_CONNECTION_REFUSED') || msg.includes('Failed to fetch')) {
            diagnostic = '🔴 VUTO: Cloudflare Worker sakuyankha. Yang\'anani /health ndi kuti PLAYER_BASE_URL ndi yolondola.';
        } else if (msg.includes('Session API')) {
            diagnostic = '🔴 VUTO: player-session-api ikulephera. Yang\'anani Supabase function logs.';
        } else if (msg.includes('stalled')) {
            diagnostic = '🔴 VUTO: R2 kapena Worker sakutumiza data. Yang\'anani Worker logs ndi R2 binding.';
        } else if (msg.includes('MEDIA_ERR_SRC_NOT_SUPPORTED')) {
            diagnostic = '🔴 VUTO: Video format siyikugwirizana kapena URL yolakwika.';
        } else {
            diagnostic = '🔴 VUTO: Onani browser console kuti muone zambiri.';
        }
        ...
        container.innerHTML = `
            <div class="error-state">
                ...
                <p style="color:#ffb3b3;font-size:0.95rem;max-width:600px;word-break:break-word;">${msg.replace(/\n/g, '<br>')}</p>
