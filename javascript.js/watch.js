// watch.js (Updated: secure HLS + HMAC signing, supports HLS + MP4)
(function() {
    'use strict';
    ...
    //   Fetches the file with HMAC signature headers (needed for Worker auth),
    //   converts to blob URL, sets as <video src>.
    //   NOTE: This loads the whole file into memory. Works for small/medium files.
    async function attachRawVideo(url) {
        destroyHls();
        destroyBlobUrl();

        if (!currentClientKey) {
            // No client key → try direct src (Worker will likely reject)
            video.src = url;
            video.load();
            return;
        }
        ...
        try {
            ...
            // Fallback: direct src (may fail if Worker requires signature)
            video.src = url;
            video.load();
            return;
        }
    ...
    // ================== 🆕 DETECT MEDIA TYPE ==================
    //   Returns true if session indicates HLS (.m3u8), false otherwise
    function isHlsSession(session) {
        if (!session) return false;

        // Explicit flag from backend (best)
        if (typeof session.is_hls === 'boolean') return session.is_hls;

        // Check contentType from backend
        ...
        // Fallback: guess from streamUrl / video_key
        ...
        // Default: try HLS (backward compat)
        return true;
    }
    ...
    async function init() {
        ...
        try {
            // ── Get secure session (clientKey + streamUrl)
    ...
    function scheduleSessionRefresh(expiresIn) {
        ...
        sessionRefreshTimer = setTimeout(async () => {
            try {
                ...
                // ════════════════════════════════════════════════════════
                // CHANGED: Use same HLS vs MP4 detection on refresh
                // ════════════════════════════════════════════════════════
                if (isHlsSession(fresh)) {
                    await attachHls(fresh.streamUrl);
                } else {
                    await attachRawVideo(fresh.streamUrl);
                }
                ...
                // Extend expiry window with new expiresIn
                if (fresh.expiresIn) {
