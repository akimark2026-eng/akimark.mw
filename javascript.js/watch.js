// watch.js (Updated: secure direct MP4 playback, no HLS)
(function() {
    'use strict';
    ...
    function attachVideo(url) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const onMeta = () => settle(resolve);
            const onErr = () => settle(reject, new Error('Video failed to load'));

            const timeout = setTimeout(
                () => settle(reject, new Error('Video load timeout (30s)')),
                30000,
            );
    ...
    async function init() {
        ...
        try {
            ...
            // ── Attach direct MP4 stream
            await attachVideo(session.streamUrl);
    ...
    function scheduleSessionRefresh(expiresIn) {
        ...
        sessionRefreshTimer = setTimeout(async () => {
            try {
                ...
                await attachVideo(fresh.streamUrl);
