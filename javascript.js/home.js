// home.js (v8 — Ultra-speed: cached posters, instant render, inline icons)
...
(function() {
    ...
    var FILMS_CACHE_KEY = 'akimark_films_cache_v1';
    ...
    // ════════════════════════════════════════════════════════════
    // 🆕 CACHE — Store films in localStorage for instant next visit
    // ════════════════════════════════════════════════════════════
    function loadCachedFilms() {
        try {
            var raw = localStorage.getItem(FILMS_CACHE_KEY);
            if (!raw) return null;
            var parsed = JSON.parse(raw);
            if (!parsed || !parsed.films || !parsed.timestamp) return null;
            if (Date.now() - parsed.timestamp > CACHE_TTL_MS) return null;
            if (!Array.isArray(parsed.films) || parsed.films.length === 0) return null;
            return parsed.films;
        } catch (e) {
            return null;
        }
    ...
    function saveFilmsCache(films) {
        try {
            localStorage.setItem(FILMS_CACHE_KEY, JSON.stringify({
                films: films,
                timestamp: Date.now()
            }));
        } catch (e) {
    ...
    // ════════════════════════════════════════════════════════════
    // MOVIE CARD — eager for first N posters globally, lazy for rest
    // ════════════════════════════════════════════════════════════
    function movieCardHTML(movie) {
        ...
        var isEager = globalPosterIndex < EAGER_LIMIT;
        globalPosterIndex++;

        var loadingAttr = isEager
    ...
    async function fetchMovies() {
        try {
            ...
            // Only fall back to empty state if we have no cached data
    ...
    function initInfiniteScroll() {
        ...
        if (!sentinel) return;
        // Avoid double-observing on re-init
        if (sentinel._akObserved) return;
        sentinel._akObserved = true;
    ...
    function initSearch() {
        ...
        if (!searchNavBtn || searchNavBtn._akInit) return;
        searchNavBtn._akInit = true;
    ...
    function initNavMovies() {
        ...
        if (!navMovies || navMovies._akInit) return;
        navMovies._akInit = true;
    ...
    function initApp() {
        // ════════════════════════════════════════════════════════
        // ULTRA-SPEED: render cached films instantly (no skeleton)
        // ════════════════════════════════════════════════════════
        var cachedFilms = loadCachedFilms();
        if (cachedFilms && cachedFilms.length > 0) {
            allFilms = cachedFilms.slice();
            renderTranslatorBar();
            renderAll();
            hideSkeletons();
            // hydrate UI (search, scroll, card click)
            initSearch();
            initInfiniteScroll();
        }

        // Init interactions (idempotent — safe to call twice)
        initCategories();
        initCardClick();
        initSecurity();
        initNavMovies();
        ...
        // Always refresh from network (background)
        fetchMovies();
    }
