// home.js (v5 — Production mode: optimized posters + live SVG badge)
...
(function() {
    ...
    // ════════════════════════════════════════════════════════════
    var POSTER_WIDTH = 500;      // Mobile-optimized width
    var POSTER_QUALITY = 75;     // Sweet spot for quality/size
    ...
    function optimizePosterUrl(originalUrl, width, quality) {
        ...
        // Ngati URL ikugwiritsa ntchito Supabase Storage /object/public/,
        // sinthani kukhala /render/image/public/ + query params
        ...
        // Fallback: URL si ya Supabase — bweletsa momwe iliri
        return originalUrl;
    }

    // Optimized thumbnail yaying'ono (kwa list views, mobile data)
    function optimizePosterThumb(originalUrl) {
        return optimizePosterUrl(originalUrl, 300, 70);
    }

    // Optimized hero/large poster (kwa watch page, view-film)
    function optimizePosterLarge(originalUrl) {
        return optimizePosterUrl(originalUrl, 800, 80);
    }

    // ════════════════════════════════════════════════════════════
    // 🆕 LIVE SVG BADGE (bottom bar)
    //   Imawonjezedwa dynamically — HTML yosinthidwa
    // ════════════════════════════════════════════════════════════
    function injectLiveBadge() {
        var bottomBar = document.querySelector('.bottom-bar') ||
                        document.querySelector('.bottom-nav') ||
                        document.querySelector('nav.bottom') ||
                        document.querySelector('[data-bottom-bar]');

        // Ngati bottom bar siyipezeka, yesani kuyika pa body nthawi ina
        if (!bottomBar) {
            // Yesani kuyika pa chilichonse chomwe chili ndi class yofanana
            var fallback = document.querySelector('.bottom-nav-container') ||
                           document.querySelector('.nav-bottom') ||
                           document.querySelector('footer');
            if (fallback) bottomBar = fallback;
        }

        if (!bottomBar) return;

        // Pewani kuyika kawiri
        if (bottomBar.querySelector('.akimark-live-badge')) return;

        var badge = document.createElement('div');
        badge.className = 'akimark-live-badge';
        badge.innerHTML =
            '<img src="akimark-live.svg" alt="Akimark Live" class="akimark-live-svg" ' +
        ...
        if (img) {
            img.style.cssText = [
                ...
                'object-fit:contain',
                'display:block',
                'filter:drop-shadow(0 0 4px rgba(225,29,72,0.35))'
            ].join(';');
        }
    ...
    // ════════════════════════════════════════════════════════════
    // 🆕 MOVIE CARD — optimized poster URL + responsive srcset
    // ════════════════════════════════════════════════════════════
    ...
    // ════════════════════════════════════════════════════════════
    // 🆕 EXPOSE HELPERS (kwa view-film.html, watch.html, etc.)
    //   Ngati mukufuna kugwiritsa ntchito ma optimization awa
    //   pa masamba ena, tsegulani window.AkimarkImages.
    // ════════════════════════════════════════════════════════════
