// home.js (v6 — Production: optimized posters + broken-image fallback + live badge)
...
(function() {
    ...
    //   Ngati Supabase image render ikulephera (mwachitsanzo
    //   transformations sinaitanidwe), gwiritsani ntchito original URL.
    //   Izi zimagwira ntchito pa image ILIYONSE yomwe ili ndi data-orig.
    // ════════════════════════════════════════════════════════════
    document.addEventListener('error', function(e) {
        var el = e.target;
        if (el && el.tagName === 'IMG' && el.dataset && el.dataset.orig) {
            if (el.src !== el.dataset.orig) {
    ...
    // ════════════════════════════════════════════════════════════
    // MOVIE CARD — optimized poster + data-orig fallback
    // ════════════════════════════════════════════════════════════
    function movieCardHTML(movie) {
        ...
        return '<div class="movie-card" data-id="' + movie.id + '">' +
            ...
            '<div class="poster">' +
                '<img src="' + escAttr(optimized) + '" ' +
                     'data-orig="' + escAttr(original) + '" ' +
                     'srcset="' + escAttr(thumb) + ' 300w, ' + escAttr(optimized) + ' 500w" ' +
    ...
    function renderTranslatorBar() {
        ...
        translators.forEach(function(t) {
            html += '<button class="translator-chip" data-translator="' + escAttr(t) + '">' + t + '</button>';
        });
    ...
    function initSearch() {
        ...
        if (searchNavBtn.dataset.bound === '1') return;
        searchNavBtn.dataset.bound = '1';
        ...
        searchNavBtn.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            var isOpen = searchSubheader.classList.toggle('open');
            searchNavBtn.classList.toggle('active', isOpen);
            if (isOpen) {
